#!/usr/bin/env python3
"""
Sequential ARC Validation v3 - Fixed memory management
"""
...
def process_single_problem(problem_idx: int, model, tokenizer, max_tokens: int = 30000):
    """Process a single ARC problem with memory management."""
    ...
    try:
        ...
        # Final memory cleanup - but keep response for saving
        del predicted_grid, target_grid
        clear_memory_cache()
        
        return result, response  # Save response to JSON, memory freed when function ends
...
def run_sequential_validation_memory_optimized(start_idx: int = 0, num_problems: int = 10, gpu: int = 5, max_tokens: int = 30000):
    """Run validation problems sequentially with proper memory management."""
    
    if not HAS_ARC:
        log_with_timestamp("❌ ARC module not available!")
        return
    
    log_with_timestamp("🚀 STARTING MEMORY-OPTIMIZED ARC VALIDATION")
    ...
    try:
        ...
        results = []
        responses = {}  # Store responses for JSON
        correct_count = 0
        ...
        for i in range(start_idx, start_idx + num_problems):
            ...
            # Store response for JSON saving
            if response:
            ...
            if i < start_idx + num_problems - 1:
                log_with_timestamp("⏱️ Brief pause and memory cleanup...")
                clear_memory_cache()
                time.sleep(3)
        ...
        log_with_timestamp("\n" + "=" * 80)
        log_with_timestamp("🎉 MEMORY-OPTIMIZED VALIDATION COMPLETE!")
        log_with_timestamp("=" * 80)
        ...
        final_data = {
            ...
            "responses": responses  # Include all responses
        }
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Sequential ARC Validation v3 - Memory Optimized")
    ...
    parser.add_argument("--tokens", type=int, default=30000, help="Max tokens")
    
    args = parser.parse_args()
    
    run_sequential_validation_memory_optimized(args.start, args.num, args.gpu, args.tokens)