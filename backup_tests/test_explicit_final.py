#!/usr/bin/env python3
"""
Test GPT-OSS with explicit final channel switching instructions
"""

import os
import json
import torch
from transformers import AutoModelForCausalLM, AutoTokenizer
from arc import train_problems
import numpy as np
from datetime import datetime
from typing import List, Dict, Any

# Set GPU
os.environ["CUDA_VISIBLE_DEVICES"] = "5"

def grid_to_string(grid: np.ndarray) -> str:
    """Convert numpy grid to string format"""
    return '\n'.join(' '.join(str(int(cell)) for cell in row) for row in grid)

def parse_grid_from_response(response: str) -> np.ndarray:
    """Extract grid from response text"""
    lines = response.strip().split('\n')
    grid_rows = []
    
    for line in lines:
        line = line.strip()
        if not line:
            continue
        
        # Skip non-grid lines
        if any(skip in line.lower() for skip in ['grid:', 'output:', '```', 'channel', 'message', 'return', 'user', 'assistant', 'analysis', 'pattern', 'example']):
            continue
        
        parts = line.split()
        if parts:
            try:
                row = []
                for part in parts:
                    if part.isdigit() or (part.startswith('-') and part[1:].isdigit()):
                        num = int(part)
                        if 0 <= num <= 9:
                            row.append(num)
                
                if row and len(row) > 1:  # Valid row should have multiple elements
                    grid_rows.append(row)
                elif grid_rows and len(row) != len(grid_rows[-1]):
                    # Stop if row length changes
                    break
                    
            except:
                if grid_rows:
                    break
                continue
    
    if grid_rows:
        # Check consistent row length
        row_lengths = [len(row) for row in grid_rows]
        if len(set(row_lengths)) == 1:
            return np.array(grid_rows)
    
    return None

def test_explicit_final():
    """Test with explicit final channel switching instructions"""
    
    print("🚀 Loading GPT-OSS model...")
    
    # Load model and tokenizer
    tokenizer = AutoTokenizer.from_pretrained(
        "openai/gpt-oss-20b",
        trust_remote_code=True,
    )
    
    model = AutoModelForCausalLM.from_pretrained(
        "openai/gpt-oss-20b",
        device_map="auto",
        torch_dtype=torch.bfloat16,
        trust_remote_code=True,
        low_cpu_mem_usage=True,
    )
    
    print("✅ Model loaded")
    print("="*70)
    
    # Enhanced system setup with explicit channel instructions
    system_setup = """<|start|>system<|message|>You are ChatGPT, a large language model trained by OpenAI.
Knowledge cutoff: 2024-06

Reasoning: high

# Valid channels: analysis, commentary, final. Channel must be included for every message.
# Channel Usage Rules:
# - Use 'analysis' channel for reasoning, thinking, and step-by-step analysis
# - Use 'final' channel for delivering the completed answer to the user
# - When you finish reasoning and are ready to provide the final result, YOU MUST switch to the 'final' channel<|end|><|start|>developer<|message|># Instructions

You are an expert at solving Abstract Reasoning Corpus (ARC) puzzles. 

## Channel Switching Protocol:
1. Use the analysis channel for reasoning and pattern identification
2. When you complete your analysis and are ready to output the grid, SWITCH to the final channel
3. In the final channel, output ONLY the grid numbers (no explanations, just the grid)

## Grid Output Format:
When switching to final channel, provide the grid as:
- Numbers separated by spaces
- One row per line
- No additional text or explanations<|end|>"""
    
    # Test first problem
    problem = train_problems[0]
    print(f"🧩 Testing Problem: {problem.uid}")
    print("="*70)
    
    # Initialize conversation
    conversation = system_setup
    turn_responses = []
    
    # Prepare training examples
    examples_text = []
    for i, train_pair in enumerate(problem.train_pairs):
        examples_text.append(
            f"Example {i+1}:\nInput:\n{grid_to_string(train_pair.x)}\nOutput:\n{grid_to_string(train_pair.y)}"
        )
    
    all_examples = "\n\n".join(examples_text)
    test_input_grid = problem.test_pairs[0].x
    target_output_grid = problem.test_pairs[0].y
    
    # ========== TURN 1: Analysis with explicit final instruction ==========
    print("\n📝 TURN 1: Analyze and produce grid with explicit channel switching")
    
    turn1_prompt = f"""<|start|>user<|message|>Solve this ARC puzzle with {len(problem.train_pairs)} training examples:

{all_examples}

Test Input:
{grid_to_string(test_input_grid)}

Instructions:
1. First use the analysis channel to identify the transformation pattern
2. Once you understand the rule, IMMEDIATELY switch to the final channel
3. In the final channel, output ONLY the complete grid for the test input (numbers only, no text)

Remember: After your analysis is complete, you MUST switch to the final channel to provide the grid output.<|end|>"""
    
    conversation += turn1_prompt + "<|start|>assistant"
    
    # Generate response
    input_ids = tokenizer(conversation, return_tensors="pt").input_ids.to(model.device)
    
    with torch.no_grad():
        output = model.generate(
            input_ids,
            max_new_tokens=3072,  # More tokens for analysis + grid
            temperature=0.7,
            top_p=0.9,
            do_sample=True,
            pad_token_id=tokenizer.pad_token_id,
            eos_token_id=[200002],  # <|return|>
        )
    
    response1 = tokenizer.decode(output[0][input_ids.shape[1]:], skip_special_tokens=False)
    conversation += response1
    turn_responses.append({
        "turn": 1,
        "type": "explicit_final_instruction",
        "response": response1,
        "has_analysis_channel": "<|channel|>analysis" in response1,
        "has_final_channel": "<|channel|>final" in response1,
        "has_return": "<|return|>" in response1
    })
    
    print(f"   ✓ Has analysis channel: {'<|channel|>analysis' in response1}")
    print(f"   ✓ Has final channel: {'<|channel|>final' in response1}")
    print(f"   ✓ Has return token: {'<|return|>' in response1}")
    
    # ========== TURN 2: If no final channel, explicit request ==========
    if "<|channel|>final" not in response1:
        print("\n📝 TURN 2: Explicit final channel request")
        
        turn2_prompt = f"""<|start|>user<|message|>You provided good analysis but didn't switch to the final channel. 

Now please:
1. Switch to the final channel
2. Output the complete {target_output_grid.shape[0]}×{target_output_grid.shape[1]} grid for the test input
3. Use the pattern you identified: diagonal arrangement based on non-zero count

Format: numbers separated by spaces, one row per line, no other text.<|end|>"""
        
        conversation += turn2_prompt + "<|start|>assistant"
        
        # Generate Turn 2
        input_ids = tokenizer(conversation, return_tensors="pt").input_ids.to(model.device)
        
        with torch.no_grad():
            output = model.generate(
                input_ids,
                max_new_tokens=2048,
                temperature=0.7,
                top_p=0.9,
                do_sample=True,
                pad_token_id=tokenizer.pad_token_id,
                eos_token_id=[200002],
            )
        
        response2 = tokenizer.decode(output[0][input_ids.shape[1]:], skip_special_tokens=False)
        conversation += response2
        turn_responses.append({
            "turn": 2,
            "type": "explicit_final_request",
            "response": response2,
            "has_analysis_channel": "<|channel|>analysis" in response2,
            "has_final_channel": "<|channel|>final" in response2,
            "has_return": "<|return|>" in response2
        })
        
        print(f"   ✓ Has analysis channel: {'<|channel|>analysis' in response2}")
        print(f"   ✓ Has final channel: {'<|channel|>final' in response2}")
        print(f"   ✓ Has return token: {'<|return|>' in response2}")
        
        # Use second response for grid extraction
        final_response = response2
    else:
        final_response = response1
    
    # Try to extract grid from final response
    extracted_grid = parse_grid_from_response(final_response)
    
    if extracted_grid is not None:
        print(f"   ✅ Extracted grid shape: {extracted_grid.shape}")
        
        if extracted_grid.shape == target_output_grid.shape:
            accuracy = np.mean(extracted_grid == target_output_grid)
            print(f"   📊 Accuracy: {accuracy:.1%}")
        else:
            print(f"   ⚠️ Shape mismatch: expected {target_output_grid.shape}")
            accuracy = 0.0
    else:
        print(f"   ❌ Could not extract grid from response")
        accuracy = 0.0
        extracted_grid = None
    
    # Store results
    problem_result = {
        "problem_id": problem.uid,
        "test_input": test_input_grid.tolist(),
        "target_output": target_output_grid.tolist(),
        "model_output": extracted_grid.tolist() if extracted_grid is not None else None,
        "accuracy": float(accuracy),
        "channels_used": {
            "has_final_channel": any("<|channel|>final" in resp["response"] for resp in turn_responses),
            "final_channel_turns": [i+1 for i, resp in enumerate(turn_responses) if "<|channel|>final" in resp["response"]]
        },
        "turn_responses": turn_responses,
        "num_turns": len(turn_responses)
    }
    
    # Save conversation
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    conv_file = f"/tmp/explicit_final_{problem.uid}_{timestamp}.txt"
    with open(conv_file, 'w') as f:
        f.write(f"EXPLICIT FINAL CHANNEL TEST\n")
        f.write(f"Problem: {problem.uid}\n")
        f.write(f"Accuracy: {accuracy:.1%}\n")
        f.write(f"Number of turns: {len(turn_responses)}\n")
        f.write(f"Final channel used: {problem_result['channels_used']['has_final_channel']}\n")
        f.write(f"Final channel in turns: {problem_result['channels_used']['final_channel_turns']}\n")
        f.write("="*70 + "\n\n")
        f.write(conversation)
        f.write("\n" + "="*70 + "\n")
        f.write("TARGET OUTPUT:\n")
        f.write(grid_to_string(target_output_grid))
        if extracted_grid is not None:
            f.write("\n" + "="*70 + "\n")
            f.write("MODEL OUTPUT:\n")
            f.write(grid_to_string(extracted_grid))
    
    print(f"\n💾 Saved to: {conv_file}")
    
    # Save JSON results
    json_file = f"/tmp/explicit_final_results_{timestamp}.json"
    with open(json_file, 'w') as f:
        json.dump(problem_result, f, indent=2)
    
    print(f"📊 Results saved to: {json_file}")
    
    return conv_file, json_file

if __name__ == "__main__":
    conv_file, json_file = test_explicit_final()