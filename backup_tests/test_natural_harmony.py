#!/usr/bin/env python3
"""
Natural harmony prompt test - let the model decide channel switching naturally
"""
...
def test_natural_harmony():
    """Test with natural harmony prompt - no manual injection"""
    
    print("🚀 Loading GPT-OSS model for natural harmony test...")
...
"""
    ...
    print("🧪 Testing natural harmony prompt (HIGH reasoning)")
    
    # Create conversation using harmony library - NATURAL WAY
    try:
...
## WORKFLOW:
1. First, use the analysis channel to understand the pattern
2. Then, use the final channel to output ONLY the grid (numbers separated by spaces, one row per line)
3. NO explanations in the final channel - only the grid

## Example format:
<|channel|>analysis<|message|>
[Your analysis of the pattern here]
...
0 6 7 8 9"""
        ...
        user_msg = Message.from_role_and_content(
            Role.USER,
            f"""Solve this ARC puzzle using the given pattern:

{pattern_example}

Please analyze the pattern first, then generate the 20×20 output grid."""
        )
        ...
        print("   ✅ Conversation created with natural harmony format")
        
        # Render conversation for completion - NO MANUAL INJECTION
        tokens = enc.render_conversation_for_completion(
            conversation,
            Role.ASSISTANT
        )
        
        print(f"   🎯 Rendered for natural completion")
        ...
        print("📊 NATURAL HARMONY ANALYSIS")
        print("=" * 50)
        ...
        conv_file = f"natural_harmony_{problem.uid}_{timestamp}.txt"
        with open(conv_file, 'w') as f:
            f.write(f"NATURAL HARMONY TEST\n")
            f.write(f"Problem: {problem.uid}\n")
            f.write(f"Library: openai-harmony (natural prompt)\n")
        ...
        print(f"\n💾 Natural harmony log saved to: {conv_file}")
        
        # Save JSON results
        json_file = f"natural_harmony_results_{timestamp}.json"
        ...
        # Final summary
        print("\n" + "=" * 70)
        print("📊 NATURAL HARMONY SUMMARY")
        print("=" * 70)
        print(f"🧩 Problem: {problem.uid}")
        print(f"🔧 Method: Natural harmony (no manual injection)")
        print(f"🧠 Reasoning: HIGH")
        print(f"📝 Response length: {structure_analysis['total_length']} chars")
        print(f"🔄 Channels found: {structure_analysis['channels_found']}")
        print(f"🔄 Transitions: {len(structure_analysis['channel_transitions'])}")
        print(f"🔢 Grid extraction: {'✅' if extracted_grid is not None else '❌'}")
        print(f"📊 Accuracy: {accuracy:.1%}")
        
        return conv_file, json_file, result
        
    except Exception as e:
        print(f"❌ Error during natural harmony test: {str(e)}")
        import traceback
        traceback.print_exc()
        return None, None, None

if __name__ == "__main__":
    conv_file, json_file, result = test_natural_harmony()